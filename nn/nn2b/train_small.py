...
def test_valid_shapes():
    for x, _, y in TrainBatchGenerator(batch_size=64, small_data=small_data, surr=surr):
...
def train(model_path="models/small_nn2b.pth", device=None, verbose=False):
    if device is None:
    ...
    for epoch in tqdm.trange(num_epochs, disable=not verbose):
        ...
        for x, mask, y in TrainBatchGenerator(
            batch_size=64, small_data=small_data, surr=surr, small_words=True
        ):
            optimizer.zero_grad()
